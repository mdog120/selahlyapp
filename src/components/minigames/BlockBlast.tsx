...
// Yellow, Pink, Green, and Purple block colors matching the Selahly aesthetic
const BLOCK_COLORS = [
    "bg-amber-100 border-amber-300 text-amber-800 shadow-[inset_0_1.5px_0_rgba(255,255,255,0.65)]",   // Yellow
    "bg-pink-100 border-pink-200 text-pink-850 shadow-[inset_0_1.5px_0_rgba(255,255,255,0.65)]",     // Pink
    "bg-emerald-100 border-emerald-300 text-emerald-850 shadow-[inset_0_1.5px_0_rgba(255,255,255,0.65)]", // Green
    "bg-purple-100 border-purple-200 text-purple-850 shadow-[inset_0_1.5px_0_rgba(255,255,255,0.65)]" // Purple
];

// Christian grace words for clears
...
export function BlockBlast() {
    ...
    return (
        <div className="flex flex-col items-center max-w-lg mx-auto w-full select-none animate-fade-in">
            {/* Inline Styles for floating clear texts */}
            <style jsx global>{`
                @keyframes floatFadeOut {
                    0% { transform: translateY(0) scale(0.9); opacity: 0; }
                    10% { transform: translateY(-5px) scale(1.05); opacity: 1; }
                    90% { transform: translateY(-35px) scale(1); opacity: 1; }
                    100% { transform: translateY(-45px) scale(0.95); opacity: 0; }
                }
                .animate-float-fade-out {
                    animation: floatFadeOut 1.2s cubic-bezier(0.25, 1, 0.50, 1) forwards;
                }
            `}</style>
            ...
            <div className="w-full flex items-center justify-between bg-white/50 border border-warm-grey/5 p-4 rounded-2xl mb-6 shadow-sm">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <Button variant="ghost" size="sm" onClick={resetGame} className="w-9 h-9 p-0 rounded-full bg-white/60 hover:bg-white shadow-sm flex items-center justify-center hover:rotate-180 transition-transform duration-500">
            ...
            <div className="relative w-full aspect-square bg-stone-100/50 rounded-3xl p-3 border border-stone-200/30 shadow-inner mb-6">
                <div className="w-full h-full grid grid-cols-8 gap-1.5">
                    {grid.map((row, r) =>
                        row.map((cellColorIdx, c) => {
                            ...
                            let cellClass = "bg-white border border-stone-200/20 hover:bg-stone-50/50";
                            if (isFilled) {
                                cellClass = BLOCK_COLORS[cellColorIdx!];
                            } else if (preview) {
                                cellClass = preview.fits
                                    ? "bg-sage-green/45 border border-sage-green/60 animate-pulse z-10"
                                    : "bg-red-100/60 border border-red-300 z-10";
                            }
                ...
                {floatingTexts.map(t => (
                    <div
                        ...
                        className="absolute text-xs font-serif font-bold text-muted-rose bg-white/95 border border-pink-100 px-3 py-1 rounded-full shadow-md pointer-events-none animate-float-fade-out z-20 select-none"
                    >
                        ✨ {t.text} ✨
                    </div>
                ))}
                ...
                {/* Game Over Screen */}
                {gameOver && (
                    <div className="absolute inset-0 bg-white/95 backdrop-blur-sm rounded-3xl flex flex-col items-center justify-center gap-4 text-center animate-fade-in z-25 p-6">
                        ...
                        <Button onClick={resetGame} className="bg-warm-cocoa text-white px-6 py-2.5 rounded-2xl font-serif text-xs font-bold tracking-wide shadow-md hover:scale-105 active:scale-95 transition-all">
                            Play Again 🌸
                        </Button>
                    </div>
                )}
                ...
                {availableShapes.map((shape, idx) => {
                    ...
                    return (
                        <div
                            ...
                            className={`aspect-square flex items-center justify-center p-3 rounded-2xl cursor-pointer transition-all duration-300 border-2 select-none active:scale-[0.97] ${
                                isSelected
                                    ? "bg-soft-blush/40 border-muted-rose shadow-md scale-105"
                                    : "bg-white/70 hover:bg-white border-transparent hover:border-warm-grey/10 shadow-sm"
                            }`}
                        >
